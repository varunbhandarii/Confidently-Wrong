...
export default function ShareMemeButton({ memeUrl, title }: ShareMemeButtonProps) {
  ...
  return (
    <button
      ...
      className="inline-flex min-h-11 items-center justify-center rounded-lg border border-[var(--border)] bg-[var(--surface-raised)] px-4 py-2.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)] transition-all hover:border-[var(--accent)] hover:text-[var(--accent)]"
    >
      {label}
    </button>
  );
